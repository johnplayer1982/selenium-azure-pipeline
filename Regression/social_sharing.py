...
def runTest(baseUrl, driver, browser):
    ...
    for article in articles:
        ...
        socialSharing = driver.find_element(By.CSS_SELECTOR, ".sharing")
        if socialSharing:
            ...
            # Check the title
            sharingTitle = driver.find_element(By.CSS_SELECTOR, ".cmp-sharing__text").text
            print(sharingTitle)
            print(title)
            assert title in sharingTitle
            print("- Component title as expected: {title}".format(title=title))

            # Check the more options link
            moreOptionsLink = driver.find_element(By.CSS_SELECTOR, ".cmp-sharing__more-options-text")
            ...
            moreOptionsPanel = driver.find_element(By.CSS_SELECTOR, ".cmp-sharing__expandable-wrapper-social")
            ...
            copyInput = driver.find_element(By.CSS_SELECTOR, ".cmp-sharing__copy-this-link-textbox-input")
            assert copyInput.get_attribute("value") == iterationUrl
            print("- 'Copy this link' contains current url")

            # Close the menu
            moreOptionsClose = driver.find_element(By.CSS_SELECTOR, ".cmp-sharing__expandable-close-link")
            ...
            # Email dropdown
            emailBtn = driver.find_element(By.CSS_SELECTOR, "#email-button")
            ...
            emailDropdown = driver.find_element(By.CSS_SELECTOR, ".cmp-sharing__expandable-wrapper-email")
            ...
            emailInput = driver.find_element(By.CSS_SELECTOR, "#copy-this-link-textbox-email")
            assert emailInput.get_attribute("value") == driver.current_url
            print("- Email input field contains the correct URL")

            emailClose = emailDropdown.find_element(By.CSS_SELECTOR, ".cmp-sharing__expandable-close-link")
            emailClose.click()
            time.sleep(0.6)
            assert emailDropdown.get_attribute("style") == "display: none;"
            print("- Email dropdown closed")
            # Hit the homepage before visiting the next page
            driver.get(baseUrl)
